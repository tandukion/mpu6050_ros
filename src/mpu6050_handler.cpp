...
void MPU6050Handler::SetRotationMatrix(Eigen::Matrix3f M) {
  rotation_matrix_ = M;
  ROS_INFO_STREAM("Set Rotaton Matrix to:\n" << rotation_matrix_);
}
...
void MPU6050Handler::UpdateData(int16_t rate) {
  ...
  while (ros::ok()) {
    ...
    // Rotate the vector of accel and gyro data with the rotated frame (based on rotation matrix)
    transformation::RotateFrame(rotation_matrix_, gyro);
    transformation::RotateFrame(rotation_matrix_, accel);
